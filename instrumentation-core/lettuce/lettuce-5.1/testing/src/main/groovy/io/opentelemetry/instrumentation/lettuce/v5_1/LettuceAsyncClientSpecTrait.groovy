...
trait LettuceAsyncClientSpecTrait {
  ...
  abstract RedisClient createRedisClient(String embeddedUrl)
  ...
  def setup() {
    redisClient = createRedisClient(embeddedDbUri)
  ...
  def "get non existent key command with handleAsync and chained with thenApply"() {
    ...
    BiFunction<String, Throwable, String> firstStage = [
      apply  : { String res, Throwable throwable ->
        conds.evaluate {
          assert res == null
  ...
  def "command with no arguments using a biconsumer"() {
    ...
    BiConsumer<String, Throwable> biConsumer = [
      accept : { String keyRetrieved, Throwable throwable ->
        conds.evaluate {
    ...
    hmsetFuture.thenApplyAsync([
      apply  : { String setResult ->
        ...
        hmGetAllFuture.thenAccept([
          accept : { Map<String, String> hmGetAllResult ->
            conds.evaluate {
